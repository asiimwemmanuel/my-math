...
class PolyTrend:
    ...
    def polyfind(
        self, degrees: List[int], known_data: List[Tuple[float, float]]
        ...
        if best_reg is None or best_poly_features is None:
            raise ValueError(
                "All degrees had a lower score than negative infinity. You might have discovered a new function..."
            )
        else:
            print(f"Best R-squared score for given degree range: {best_score}")
            return lambda x_vals: best_reg.predict(
