...
class PolyTrendTest(unittest.TestCase):
    ...
    def generate_data(
        ...
        err_lower_bound: float,
        err_upper_bound: float,
        ...
        data = [
            (
                x,
                float(np.polyval(coeffs, x))
                + random.uniform(err_lower_bound, err_upper_bound),
            )
            for x in [
                random.uniform(data_lower_bound, data_upper_bound)
